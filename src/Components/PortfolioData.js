import travel from "../scroll/travel.jpg";
import travel1 from "../scroll/travel.jpg";
import auroglow from "../scroll/auroglow.jpg";
import auroglow1 from "../scroll/aurar.png";
import lashes from "../scroll/lashes.png";
import lashes1 from "../scroll/lashes.png";
import blue from "../scroll/blue.png";
import blue1 from "../scroll/blue.png";
import ignition from "../scroll/igni.png";
import ignition1 from "../scroll/ignition.png";
import notary from "../scroll/notary.png";
import notary1 from "../scroll/notary.png";
import cheelux from "../scroll/cheelux.png";
import cheelux1 from "../scroll/cheelux.png";
import triumph from "../scroll/triumph.png";
import triumph1 from "../scroll/triumph.png";

export const portfolioData = [
  {
    id: "SunPro",
    name: "SunPro",
    image: travel,
    imageResponsive: travel1,
    description: "Embark on a Journey with Our Travel Agency Website",
    review: {
      name: "Emily Stevens",
      rating: 4.8,
      date: "December 5, 2023",
      content:
        "SunPro's website design is like a passport to magical destinations. It captures the essence of travel, providing a visual escape. I'm thrilled with their creativity and attention to detail.",
    },
  },
  {
    id: "AuraGlow",
    name: "AuraGlow",
    image: auroglow,
    imageResponsive: auroglow1,
    description: "Glow from Within with Our Skincare Website",
    review: {
      name: "Alexander Ford",
      rating: 4.6,
      date: "December 10, 2023",
      content:
        "AuraGlow's website is a sanctuary for skincare enthusiasts. The design radiates sophistication and mirrors the quality of their products. An exquisite user experience indeed!",
    },
  },
  {
    id: "K-nicoleLashes",
    name: "K-nicole Lashes",
    image: lashes,
    imageResponsive: lashes1,
    description: "Transform Your Look with Our Lashes Website",
    review: {
      name: "Kristen Parker",
      rating: 4.5,
      date: "December 15, 2023",
      content:
        "K-nicole Lashes' website is a beauty haven! It goes beyond lashes, offering an immersive beauty experience. The team's creativity has truly elevated their brand.",
    },
  },
  {
    id: "JointA",
    name: "JointA",
    image: blue,
    imageResponsive: blue1,
    description: "Dive into the Crypto World with Our Website",
    review: {
      name: "Oliver Thompson",
      rating: 4.9,
      date: "December 20, 2023",
      content:
        "JointA's crypto website is a digital fortress! It seamlessly combines visual appeal, security, and user-friendliness. The team's expertise in crypto web design is commendable.",
    },
  },
  {
    id: "ignitionCarcare",
    name: "ignition Carcare",
    image: ignition,
    imageResponsive: ignition1,
    description: "Rev Up Your Auto Experience with Our Website",
    review: {
      name: "Eva Rodriguez",
      rating: 4.8,
      date: "December 25, 2023",
      content:
        "ignition Carcare's website is a game-changer! The design is practical, and the appointment booking system streamlines their services. Kudos to the team for their automotive expertise!",
    },
  },
  {
    id: "TsgNotary",
    name: "TsgNotary",
    image: notary,
    imageResponsive: notary1,
    description: "Elevate Your Notary Services with Our Website",
    review: {
      name: "David Lewis",
      rating: 4.7,
      date: "December 30, 2023",
      content:
        "TsgNotary's website design exceeded my expectations! It's professional, user-friendly, and aligns perfectly with the notary services they offer. Highly satisfied with the outcome!",
    },
  },
  {
    id: "cheelux",
    name: "cheelux",
    image: cheelux,
    imageResponsive: cheelux1,
    description: "Embrace Fitness with Our Gym Wear Website",
    review: {
      name: "Caroline Turner",
      rating: 4.4,
      date: "January 5, 2024",
      content:
        "Cheelux is not just gym wear; it's a lifestyle! The website design is trendy, offering a seamless online shopping experience. The team's dedication to fitness is evident. Love it!",
    },
  },
  {
    id: "TriumphSolarPicasso",
    name: "Triumph SolarPicasso",
    image: triumph,
    imageResponsive: triumph1,
    description: "Harness the Power of the Sun with Our Solar Website",
    review: {
      name: "Tom Harper",
      rating: 4.7,
      date: "January 10, 2024",
      content:
        "Triumph SolarPicasso's website design impressed me! It's informative, visually appealing, and user-friendly. The team's commitment to sustainability shines through. Excellent work!",
    },
  },
];
